...
const WHATSAPP_LINK = `https://wa.me/message/${WHATSAPP_CODE}`
...
const WHATSAPP_LINK_PREFILLED = (text) => `https://api.whatsapp.com/message/${WHATSAPP_CODE}?text=${encodeURIComponent(text)}`
...
/* ═══════════════════════════════════════════════
   TESTIMONIALS — generic placeholder quotes, no fabricated
   names/photos presented as real people. Swap for real
   customer quotes before this goes live long-term.
═══════════════════════════════════════════════ */
const TESTIMONIALS = [
  {
...
function TestimonialCard({ quote, role, rating, index }) {
  const { reducedMotion } = usePerfTier()
  const tilt = useTilt3D()
  return (
    <motion.div
...
/* ═══════════════════════════════════════════════
   GET IN TOUCH — email + feedback, routed to WhatsApp
   (static site, no backend to receive submissions)
═══════════════════════════════════════════════ */
function ContactSection() {
  ...
  const handleSend = (e) => {
    ...
    window.open(WHATSAPP_LINK_PREFILLED(`Hi AR Labs!\n${lines.join('\n')}`), '_blank', 'noopener,noreferrer')
    setEmail(''); setMessage('')
  }
  ...
  return (
    <section className="contact" id="contact">
      ...
      <motion.form className="contact-form" onSubmit={handleSend}
